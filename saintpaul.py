...
duration = 0.1    # The duration of time in seconds between each frame.
cycles = 30      # The number of cycles the animation plays through.
...
def animate():
    ...
    while count < cycles:
        for frame in animation.frames:
            print ('\033[1;31m' + frame + '\033[1;m')  # Print the frame in color blue.
            time.sleep(duration)
...
def post(event,proxy_type):
	...
	while True:
		try:
			...
			s.close()
	nums = 0
def checking(lines,proxy_type,ms,rlock,):#Proxy checker coded by S4int Paul
	global nums
	global proxies
...
def check_socks(ms):#Coded by S4int Paul
	global nums
	thread_list=[]
...
def PrintHelp():
	print('''===============  Saint Paul DDoS help list  ===============
